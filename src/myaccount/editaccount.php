	...
	$country=$row['country'];
			...
			if(!empty($_POST['user_fname']))
		{
			$fname=$_POST["user_fname"];
						...
						if (   (!preg_match("/^[a-zA-Z]*$/",$fname)) ) 
							...
							$fname_edit=$_POST['user_fname'];
							$query_fname="update customer set fname='$fname' where email='$email_myaccount'";
							$result_update1=mysqli_query($con, $query_fname);
						}
		...
		if(!empty($_POST['user_lname'])) 
		{
			...
			if ( (!preg_match("/^[a-zA-Z]*$/",$lname)) ) 
							...
							$lname_edit=$_POST["user_fname"];
							$query_lname="update customer set lname='$lname' where email='$email_myaccount'";
							$result_update2=mysqli_query($con, $query_lname);
						}
		...
		{
			$number_edit=$_post['user_number'];
			$query_number="update customer set mobile='$number_edit' where email='$email_myaccount'";	
			$result_update3=mysqli_query($con, $query_number);
		}
		if(!empty($_post['user_state']))
		{
			$state_edit=$_POST['user_state'];
			$query_state="update customer set state='$state_edit' where email='$email_myaccount'";
			$result_update4=mysqli_query($con, $query_state);
		}
		if(!empty($_POST['user_address']))
		{
			$address_edit=$_POST['user_address'];
			$query_address="update customer set address1='$address_edit' where email='$email_myaccount'";
			$result_update5=mysqli_query($con, $query_address);
		}	
		
		if(!empty($_POST['user_country']))
		{
			$country_edit=$_POST['user_country'];
			$query_address="update customer set country='$country_edit' where email='$email_myaccount'";
			$result_update5=mysqli_query($con, $query_address);
		}
		
			
		if( (!empty($_POST['user_fname']))|| (!empty($_POST['user_lname'])) ||  (!empty($_post['user_number'])) || (!empty($_post['user_state']))|| (!empty($_POST['user_address'])))
		{
...
<div class="outer" style=" color:blue;border: solid; border-color:#4fccc6; height:1000px; width: 1300px; margin-left: 80px; margin-top: 99px;">
	
	<div  class="one" style="position: absolute; margin-top: 100px; margin-left: 50px;width:300px; height:500px">
		
		
		<img width="225px" height="225px" style=" margin-left:40px;"  src="images.jpg"/>
		<div>
			<form>
			<input class="upload"  type="file" size="30"  style=" color:white;margin-left:40px; "/><br /><br />
	...
	<div class="one" style=" position: absolute; margin-top: 100px; margin-left: 600px; width:600px; height:700px;">
		
		
		<form  class="reg" name="reg"   method="post"   action="" style="padding-left: 150px">
	  			...
	  			<tr><td><input class="button" type="submit" name="submit_user" size="30" value="save"/><br><br></td></tr>
		...
		<table style="background: transparent;">
			...
			<tr width="100%">
				<td style="margin-left: 30px; margin-right: 20px; padding-left: 40px; padding-right: 40px;">
					<a class="a2" href="../static/aboutus.html">About SoundHaven</a><br>
					<a class="a2" href="../static/termsofuse.html">Terms of Use</a><br>
					<a class="a2" href="../static/privacy.html">Privacy Policy</a>
				</td>
